...
try:
 fout1 = open("time_log_aes_eax_enc_growth.txt", "a+") 
except IOError:
    print("IO Error")
    exit()

try:
 fout2 = open("time_log_aes_eax_dec_growth.txt", "a+") 
except IOError:
    print("IO Error")
    exit()
...
while (i != 0):
    # message data
    data_byte += get_random_bytes(32)
    data = base64.urlsafe_b64encode(data_byte)
    ...
    fout1.writelines(str(len(data_byte)) + "," + str(tf) + '\n')
    ...
    try:
        ...
        tf = int(time.time_ns()) - tn
        fout2.writelines(str(len(data_byte)) + "," + str(tf) + '\n')
    except ValueError:
